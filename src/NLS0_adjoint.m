function [wbreal_ret_reshape,wbimag_ret_reshape]=NLS0_adjoint(Nx,Ny,Nt,dx,dy,dt,k,sigma,sigmaTPA,gamma,u_real,u_imag,d_real,d_imag)
...
qx = 0; % the degree for u in x-axis
qy = 0; % the degree for u in y-axis
% Nx = 20; % the number of cells in x-axis
% Ny = 20; % the number of cells in y-axis
T = Nt*dt; % the simulation time
% central flux

% set up the grid
x = linspace(0,Nx*dx,Nx+1);
hx = x(2:Nx+1) - x(1:Nx);
y = linspace(0,Ny*dy,Ny+1);
hy = y(2:Ny+1) - y(1:Ny);
...
new_k = zeros((qx+1)*(qy+1), Nx*Ny);
new_kb = zeros((qx+1)*(qy+1), Nx*Ny);
new_gamma = zeros((qx+1)*(qy+1), Nx*Ny);
new_sigmaTPA = zeros((qx+1)*(qy+1), Nx*Ny);
new_sigma = zeros((qx+1)*(qy+1), Nx*Ny);
for i=1:Nx
    new_k(:,((i-1)*Ny+1):(i*Ny)) = k(i,:);
    new_kb(:,((i-1)*Ny+1):(i*Ny)) = k(i,:);
    new_gamma(:,((i-1)*Ny+1):(i*Ny)) = gamma(i,:);
    new_sigmaTPA(:,((i-1)*Ny+1):(i*Ny)) = sigmaTPA(i,:);
    new_sigma(:,((i-1)*Ny+1):(i*Ny)) = sigma(i,:);
end
...
for i=1:Nx
    for j=1:Ny
        ...
        for d = 1:((qx+1)*(qy+1))
            w_real(d,(i-1)*Ny+j) = (wloc_real(i,j)*(W))*P(:,d);
            w_imag(d,(i-1)*Ny+j) = (wloc_imag(i,j)*(W))*P(:,d);
        end
...
for it = 1:nsteps
    ...
    [rhsw_real, rhsw_imag] = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, w_real, ...
        w_imag, new_k, new_kb, new_gamma, new_sigmaTPA, new_sigma, P, W, Sx, Sy, Wx, Wy, u_real(:,:,nsteps-it), u_imag(:,:,nsteps-it));
    ...
    [rhsw_real, rhsw_imag] = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, w_real1, ...
        w_imag1, new_k, new_kb, new_gamma, new_sigmaTPA, new_sigma, P, W, Sx, Sy, Wx, Wy, u_real(:,:,nsteps-it), u_imag(:,:,nsteps-it));
    ...
    [rhsw_real, rhsw_imag] = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, w_real2, ...
        w_imag2, new_k, new_kb, new_gamma, new_sigmaTPA, new_sigma, P, W, Sx, Sy, Wx, Wy, u_real(:,:,nsteps-it), u_imag(:,:,nsteps-it));
    ...
    [rhsw_real3, rhsw_imag3] = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, w_real3, ...
        w_imag3, new_k, new_kb, new_gamma, new_sigmaTPA, new_sigma, P, W, Sx, Sy, Wx, Wy, u_real(:,:,nsteps-it), u_imag(:,:,nsteps-it));
    ...
    [rhsw_real, rhsw_imag]   = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, w_real4, ...
        w_imag4, new_k, new_kb, new_gamma, new_sigmaTPA, new_sigma, P, W, Sx, Sy, Wx, Wy, u_real(:,:,nsteps-it), u_imag(:,:,nsteps-it));
...
wbreal_ret_reshape = zeros(Nx,Ny,nsteps+1);
wbimag_ret_reshape = zeros(Nx,Ny,nsteps+1);

for i=1:Nx
    wbreal_ret_reshape(i,:,:) = wbreal_ret(((i-1)*Ny+1):(i*Ny),:);
    wbimag_ret_reshape(i,:,:) = wbimag_ret(((i-1)*Ny+1):(i*Ny),:);
end

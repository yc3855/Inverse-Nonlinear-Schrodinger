function [ucloc_real,ucloc_imag]=INVcoeff_forward(qx,qy,Nx,Ny,T)
...
k = zeros((qx+1)*(qy+1),(Nx*Ny));
gamma = zeros((qx+1)*(qy+1),(Nx*Ny));
sigmaTPA = zeros((qx+1)*(qy+1),(Nx*Ny));
sigma = zeros((qx+1)*(qy+1),(Nx*Ny));
...
for i=1:Nx
    for j=1:Ny
        ...
        [kloc] = solk(xloc(:,i),yloc(:,j));
        k(:,(i-1)*Ny+j) = kloc;
        [gammaloc] = solgamma(xloc(:,i),yloc(:,j));
        gamma(:,(i-1)*Ny+j) = gammaloc;
        [sigmaTPAloc] = solsigmaTPA(xloc(:,i),yloc(:,j));
        sigmaTPA(:,(i-1)*Ny+j) = sigmaTPAloc;
        [sigmaloc] = solsigma(xloc(:,i),yloc(:,j));
        sigma(:,(i-1)*Ny+j) = sigmaloc;
    end
end
...
for it = 1:nsteps
    ...
    [rhsu_real, rhsu_imag] = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, u_real, ...
        u_imag, k, gamma, sigmaTPA, sigma, P, W, Px, Py, Sx, Sy, Wx, Wy, xloc, yloc);
    ...
    [rhsu_real, rhsu_imag] = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, u_real1, ...
        u_imag1, k, gamma, sigmaTPA, sigma, P, W, Px, Py, Sx, Sy, Wx, Wy, xloc, yloc);
    ...
    [rhsu_real, rhsu_imag] = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, u_real2, ...
        u_imag2, k, gamma, sigmaTPA, sigma, P, W, Px, Py, Sx, Sy, Wx, Wy, xloc, yloc);
    ...
    [rhsu_real3, rhsu_imag3] = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, u_real3, ...
        u_imag3, k, gamma, sigmaTPA, sigma, P, W, Px, Py, Sx, Sy, Wx, Wy, xloc, yloc);
    ...
    [rhsu_real, rhsu_imag]   = schrodinger_update(qx, qy, hx, hy, Nx, Ny, Vb_x, Vb_y, Uxb_x, Uxb_y, u_real4, ...
        u_imag4, k, gamma, sigmaTPA, sigma, P, W, Px, Py, Sx, Sy, Wx, Wy, xloc, yloc);
...
k = zeros(1,len_x*len_y);

for i=1:len_x
    k(1,((i-1)*len_y+1):(i*len_y)) = 2*pi;
end

end

function [gamma] = solgamma(xloc,yloc)
...
gamma = zeros(1,len_x*len_y);

for i=1:len_x
    gamma(1,((i-1)*len_y+1):(i*len_y)) = 4*pi;
end

end

function [sigmaTPA] = solsigmaTPA(xloc,yloc)
...
sigmaTPA = zeros(1,len_x*len_y);

for i=1:len_x
    sigmaTPA(1,((i-1)*len_y+1):(i*len_y)) = 0;
end

end

function [sigma] = solsigma(xloc,yloc)
...
sigma = zeros(1,len_x*len_y);

for i=1:len_x
    sigma(1,((i-1)*len_y+1):(i*len_y)) = 0;
end
...
for i=1:Nx
    for j=1:Ny
        ...
        [kb] = solk(xloc(:,i),(j-1)./Ny);
        
        Fu_real =         - (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Vb_y(1,:)')*Uxb_y(1,:)) * (-uimag_local);
        Fu_real = Fu_real - (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Vb_y(1,:)')*Uxb_y(2,:)) * (-uimag_out);
        Fu_real = Fu_real - (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Uxb_y(1,:)')*Vb_y(1,:)) * (-uimag_local);
        Fu_real = Fu_real - (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Uxb_y(1,:)')*Vb_y(2,:)) * ( uimag_out);
        
        Fu_imag =         - (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Vb_y(1,:)')*Uxb_y(1,:)) * ( ureal_local);
        Fu_imag = Fu_imag - (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Vb_y(1,:)')*Uxb_y(2,:)) * ( ureal_out);
        Fu_imag = Fu_imag - (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Uxb_y(1,:)')*Vb_y(1,:)) * ( ureal_local);
        Fu_imag = Fu_imag - (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Uxb_y(1,:)')*Vb_y(2,:)) * (-ureal_out);
        ...
        Fu_real = Fu_real + (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Vb_y(2,:)')*Uxb_y(2,:)) * (-uimag_local);
        Fu_real = Fu_real + (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Vb_y(2,:)')*Uxb_y(1,:)) * (-uimag_out);
        Fu_real = Fu_real + (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Uxb_y(2,:)')*Vb_y(2,:)) * (-uimag_local);
        Fu_real = Fu_real + (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Uxb_y(2,:)')*Vb_y(1,:)) * ( uimag_out);
        
        Fu_imag = Fu_imag + (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Vb_y(2,:)')*Uxb_y(2,:)) * ( ureal_local);
        Fu_imag = Fu_imag + (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Vb_y(2,:)')*Uxb_y(1,:)) * ( ureal_out);
        Fu_imag = Fu_imag + (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Uxb_y(2,:)')*Vb_y(2,:)) * ( ureal_local);
        Fu_imag = Fu_imag + (1/2) * kron((Px')*diag(1./kb)*diag(Wx)*Px, (Uxb_y(2,:)')*Vb_y(1,:)) * (-ureal_out);
        ...
        [kb] = solk((i-1)./Nx,yloc(:,j));
        
        Fu_real = Fu_real - (1/2) * kron((Vb_x(1,:)')*Uxb_x(1,:), (Py')*diag(1./kb)*diag(Wy)*Py) * (-uimag_local);
        Fu_real = Fu_real - (1/2) * kron((Vb_x(1,:)')*Uxb_x(2,:), (Py')*diag(1./kb)*diag(Wy)*Py) * (-uimag_out);
        Fu_real = Fu_real - (1/2) * kron((Uxb_x(1,:)')*Vb_x(1,:), (Py')*diag(1./kb)*diag(Wy)*Py) * (-uimag_local);
        Fu_real = Fu_real - (1/2) * kron((Uxb_x(1,:)')*Vb_x(2,:), (Py')*diag(1./kb)*diag(Wy)*Py) * ( uimag_out);
        
        Fu_imag = Fu_imag - (1/2) * kron((Vb_x(1,:)')*Uxb_x(1,:), (Py')*diag(1./kb)*diag(Wy)*Py) * ( ureal_local);
        Fu_imag = Fu_imag - (1/2) * kron((Vb_x(1,:)')*Uxb_x(2,:), (Py')*diag(1./kb)*diag(Wy)*Py) * ( ureal_out);
        Fu_imag = Fu_imag - (1/2) * kron((Uxb_x(1,:)')*Vb_x(1,:), (Py')*diag(1./kb)*diag(Wy)*Py) * ( ureal_local);
        Fu_imag = Fu_imag - (1/2) * kron((Uxb_x(1,:)')*Vb_x(2,:), (Py')*diag(1./kb)*diag(Wy)*Py) * (-ureal_out);
        ...
        Fu_real = Fu_real + (1/2) * kron((Vb_x(2,:)')*Uxb_x(2,:), (Py')*diag(1./kb)*diag(Wy)*Py) * (-uimag_local);
        Fu_real = Fu_real + (1/2) * kron((Vb_x(2,:)')*Uxb_x(1,:), (Py')*diag(1./kb)*diag(Wy)*Py) * (-uimag_out);
        Fu_real = Fu_real + (1/2) * kron((Uxb_x(2,:)')*Vb_x(2,:), (Py')*diag(1./kb)*diag(Wy)*Py) * (-uimag_local);
        Fu_real = Fu_real + (1/2) * kron((Uxb_x(2,:)')*Vb_x(1,:), (Py')*diag(1./kb)*diag(Wy)*Py) * ( uimag_out);
        
        Fu_imag = Fu_imag + (1/2) * kron((Vb_x(2,:)')*Uxb_x(2,:), (Py')*diag(1./kb)*diag(Wy)*Py) * ( ureal_local);
        Fu_imag = Fu_imag + (1/2) * kron((Vb_x(2,:)')*Uxb_x(1,:), (Py')*diag(1./kb)*diag(Wy)*Py) * ( ureal_out);
        Fu_imag = Fu_imag + (1/2) * kron((Uxb_x(2,:)')*Vb_x(2,:), (Py')*diag(1./kb)*diag(Wy)*Py) * ( ureal_local);
        Fu_imag = Fu_imag + (1/2) * kron((Uxb_x(2,:)')*Vb_x(1,:), (Py')*diag(1./kb)*diag(Wy)*Py) * (-ureal_out);

function [ubreal_ret_reshape,ubimag_ret_reshape]=NLS0_forward(Nx,Ny,Nt,dx,dy,dt,k,sigma,sigmaTPA,gamma,uinit)

% Solve u_t = (i/2k(x))(u_xx + u_yy) + (i gamma(x) - 1/2 sigma_TPA(x)) |u|^2 u
%            -1/2 sigma(x) u
% in 2D 0 < x < Nx * dx, 0 < y < Ny * dy by DG
% periodic boundary conditions

qx = 0; % qx = degree for u in x-axis
qy = 0; % qy = degree for u in y-axis
% Nx = 20; % Nx = number of cells in x-axis
% Ny = 20; % Ny = number of cells in y-axis
T = Nt*dt; % T is the simulation time
...
% uinit is an array of shape [Nx,Ny,2]

for i=1:Nx
    for j=1:Ny
        
        uloc = squeeze(uinit(i,j,:));
...
ubreal_ret_reshape = zeros(Nx,Ny,nsteps+1);
ubimag_ret_reshape = zeros(Nx,Ny,nsteps+1);

for i=1:Nx
    ubreal_ret_reshape(i,:,:) = ubreal_ret(((i-1)*Ny+1):(i*Ny),:);
    ubimag_ret_reshape(i,:,:) = ubimag_ret(((i-1)*Ny+1):(i*Ny),:);
end
